...
# Removes py2 support, and adds the ability to add
# rebus clues to a new file
...
class Puzzle:
    ...
    def create_empty_rebus(self):
        rebus = self.rebus()
        self.extensions[Extensions.Rebus] = b''
        self.extensions[Extensions.RebusSolutions] = b''
        rebus.fill = {}
        rebus.solutions = {}
        rebus.table = []
        return rebus
...
class Rebus:
    ...
    def add_rebus(self, answer):
        if answer is None:
            self.table.append(0)
        else:
            if answer not in self.temp:
                self.temp[answer] = len(self.temp) + 1
            self.solutions[f"{self.temp[answer]:2d}"] = answer
            self.table.append(self.temp[answer] + 1)
